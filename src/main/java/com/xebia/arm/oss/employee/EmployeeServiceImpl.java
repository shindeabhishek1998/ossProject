...
public class EmployeeServiceImpl implements EmployeeServiceInterface, OSSConstants {
	 ...
	 */
	/*
	 * @Override public EmployeeResponse getEmpDetailsByEmpId(int empId) {
	 * EmployeeResponse res = new EmployeeResponse();
	 * 
	 * EmployeeDetails ed = dao.getEmpDetailsByEmpId(empId); if (ed != null) {
	 * res.setStatusCode(STATUS_CODE_200); res.setStatus(STATUS_SUCCESS);
	 * res.setResult(RESULT_TRUE);
	 * res.setMessage("Employee record deleted successfully");
	 * res.setEmpDetails(ed); } else { res.setStatusCode(STATUS_CODE_200);
	 * res.setStatus(STATUS_FAIL); res.setResult(RESULT_FALSE);
